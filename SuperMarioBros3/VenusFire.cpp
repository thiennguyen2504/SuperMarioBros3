...
void VenusFire::Update(DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
    ...
    if (mario)
    {
        ...
        float pipeLeft = 64;
        float pipeRight = 96;
        isMarioOnPipe = (my < pipeTop && mx >= pipeLeft && mx <= pipeRight);
    }
    ...
    else if (state == VENUS_FIRE_STATE_WAITING)
    {
        ...
        if (isMarioNear && fireballTimer >= VENUS_FIRE_FIREBALL_COOLDOWN)
        {
            Fireball* fireball = new Fireball(x, y, isFacingRight);
